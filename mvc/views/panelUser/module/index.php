    ...
    <div class="row">
        <div class="col-12" id="MessageFlash">
            ...
            <?php if(isset($_SESSION['error'])) {?>
                <h3 class="text-danger"> <?= $redirect->setFlash('error'); ?> </h3>
            <?php } ?>
        </div>
    </div>
    <div class="x_content">
            ...
            <table class="table table-striped jambo_table bulk_action">
                ...
                <tbody>
                    ...
                    <tr class="even<?= $val['id'] ?> pointer">
                        ...
                        <td class="">
                            <input type="checkbox"onclick="checkPublish(<?= $val['id']?>, 'publish')" id="publish<?= $val['id']?>" data-control="<?= $data['template'] ?>" <?= $val['publish'] == 1 ? 'checked' : ''?>>
                        </td>
                        <td class="">
                            <input type="number" id="sort<?= $val['id']?>" data-control="<?= $data['template'] ?>" value="<?= $val['sort'] ?>">
                        </td>
                        <td class=""><?= date('d/m/Y', strtotime($val['create_at'])) ?></td>
                        <td>
                    ...
                    <?php if(isset($val['children']) && $val['children'] != null){?>
                        <?php foreach ($val['children'] as $key => $val_child) { ?>
                        <tr class="even<?= $val_child['id'] ?> pointer">
                            ...
                            <td class="">
                                <input type="checkbox"onclick="checkPublish(<?= $val_child['id']?>, 'publish')" id="publish<?= $val_child['id']?>" data-control="<?= $data['template'] ?>" <?= $val_child['publish'] == 1 ? 'checked' : ''?>>
                            </td>
                            <td class="">
                                <input type="number" id="sort<?= $val_child['id']?>" data-control="<?= $data['template'] ?>" value="<?= $val_child['sort'] ?>">
                            </td>
                            <td class=""><?= date('d/m/Y', strtotime($val_child['create_at'])) ?></td>
                            <td>
